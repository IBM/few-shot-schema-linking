"""Prepare database content indexes for content linking.

Adapted from the CodeS github repository. 
https://github.com/RUCKBReasoning/codes/blob/main/prepare_sft_datasets.py
"""

from dec_sl.utils.db_utils import get_cursor_from_path, execute_sql_long_time_limitation
import json
import os, shutil
...
if __name__ == "__main__":
    ...
    remove_contents_of_a_folder("./data/bird/train/db_contents_index")
    # build content index for BIRD's training set databases
    for db_id in os.listdir("./data/bird/train/train_databases"):
        if db_id.endswith(".json"):
            continue
        print(db_id)
        build_content_index(
            os.path.join("./data/bird/train/train_databases/", db_id, db_id + ".sqlite"),
            os.path.join("./data/bird/train/db_contents_index/", db_id)
        )
    
    print("build content index for BIRD's dev set databases...")
    remove_contents_of_a_folder("./data/bird/dev/db_contents_index")
    # build content index for BIRD's dev set databases
    for db_id in os.listdir("./data/bird/dev/dev_databases"):
        print(db_id)
        build_content_index(
            os.path.join("./data/bird/dev/dev_databases/", db_id, db_id + ".sqlite"),
            os.path.join("./data/bird/dev/db_contents_index/", db_id)
        )

    print("build content index for spider's databases...")
    remove_contents_of_a_folder("./data/spider/db_contents_index")
    # build content index for spider's databases
    for db_id in os.listdir("./data/spider/database"):
        print(db_id)
        build_content_index(
            os.path.join("./data/spider/database/", db_id, db_id + ".sqlite"),
            os.path.join("./data/spider/db_contents_index/", db_id)
        )
