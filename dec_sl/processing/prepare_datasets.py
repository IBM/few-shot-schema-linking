"""Prepare and pre-process a Text-to-SQL dataset.

Adapted from the CodeS github repository. 
...
from dec_sl.utils.bridge_content_encoder import get_matched_entries
from dec_sl.utils.db_utils import get_db_schema, detect_special_char
...
def extract_large_numbers(text):
    ...
    for word, multiplier in patterns.items():
        matches = re.findall(
            r"(\d+\.?\d*)\s*{}".format(word), text, flags=re.IGNORECASE
        )
...
def spider_style_dataset(
    ...
    db_id2searcher = dict()
    for db_id in db_ids:
        db_id2searcher[db_id] = LuceneSearcher(
            os.path.join(db_content_index_path, db_id)
        )
    ...
    for data in tqdm(dataset):
        ...
        # Create schema in a more detailed way
        ...
        # Get question and evidence
        if "spider-syn" in source:
            sample["question"] = data["SpiderSynQuestion"]
            sample["evidence"] = ""
        elif "bird" in source:
            sample["question"] = data["question"]
            evidence = preprocess_evidence(
                data["evidence"], sample["schema"]["schema_items"]
            )
        ...
        # Create the (input) text by concatenating question and evidence
        sample["text"] = (
            sample["evidence"] + " " + sample["question"]
            if use_evidence and sample["evidence"] != ""
            else sample["question"]
        )

        # Get SQL query, format it, and remove table aliases
        if mode in ["train", "dev"]:
            sql = data["SQL"] if source in ["bird-dev", "bird-train"] else data["query"]
            sample["sql"] = remove_table_alias(
                sqlparse.format(sql, keyword_case="upper", identifier_case="lower")
            )
        elif mode == "test":
            sample["sql"] = ""

        # Check if a table/column name needs to be inside backticks to avoid errors
        sql = sample["sql"]
        for table in sample["schema"]["schema_items"]:
            if detect_special_char(table["table_name"]):
                # Fix table names if they exist
                sql = sql.replace(table["table_name"], f"`{table['table_name']}`")
            for column_name in table["column_names"]:
        ...
        # Create schema linking labels for tables and columns
        sample["table_labels"], sample["column_labels"] = [], []
        try:
            query_tables, query_columns = extract_tables_and_columns(
                sample["sql"], schema=sample["schema"]
            )
        except:
            print(f"Error parsing SQL query: {sample['sql']}")
            query_tables, query_columns = [], []

        # Create classification labels from table and column names
        for table_info in sample["schema"]["schema_items"]:
            if mode in ["train", "dev"]:
                table_name = table_info["table_name"]
                sample["table_labels"].append(1 if table_name in query_tables else 0)
                sample["column_labels"].append(
                    [
                        (1 if f"{table_name}.{column_name}" in query_columns else 0)
                ...
                sample["column_labels"].append(
                    [0 for _ in range(len(table_info["column_names"]))]
                )
        ...
        for i in range(len(hits)):
            matched_result = json.loads(hits[i].lucene_document.get("raw"))
...
if __name__ == "__main__":
        ...
        spider_train.extend(
            spider_style_dataset(
                dataset_path=os.path.join(
                    "./data/spider/", spider_train_set
                ),
                db_path="./data/spider/database",
                db_content_index_path="./data/spider/db_contents_index",
                source="spider-train",
    ...
    with open("./data/processed/spider_train_text2sql.json", "w") as f:
    ...
    bird_train = spider_style_dataset(
        ...
        db_content_index_path="./data/bird/train/db_contents_index",
        source="bird-train",
        table_json_path="./data/bird/train/train_tables.json",
        use_evidence=False,
        mode="train",
    )
    with open("./data/processed/bird_train_text2sql.json", "w") as f:
    ...
    with open("./data/processed/bird_with_evidence_train_text2sql.json", "w") as f:
    ...
    #     f.write(
    #         json.dumps(
    #             spider_train + bird_with_evidence_train + bank_train + aminer_train,
    #             indent=2,
    #             ensure_ascii=False,
    #         )
    #     )
    ...
    #                 "./data/sft_data_collections/diagnostic-robustness-text-to-sql/data/",
    #                 test_set_name,
    #                 "questions_post_perturbation.json",
    #             ),
    ...
    with open("./data/processed/spider_dev_text2sql.json", "w") as f:
    ...
    bird_dev = spider_style_dataset(
        ...
        db_content_index_path="./data/bird/dev/db_contents_index",
        source="bird-dev",
        table_json_path="./data/bird/dev/dev_tables.json",
        use_evidence=False,
        mode="dev",
    )
    with open("./data/processed/bird_dev_text2sql.json", "w") as f:
    ...
    with open("./data/processed/bird_with_evidence_dev_text2sql.json", "w") as f:
